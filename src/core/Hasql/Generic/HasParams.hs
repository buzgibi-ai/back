...
--
-- \-- Search for a 'Person' with a matching id
...
instance HasEValue a => HasEField (Vector (Maybe a)) where
  {-# INLINE mkEField #-}
  mkEField = param $ nonNullable $ foldableArray @Vector (nullable (mkEValue @a))

instance HasEValue a => HasEField (Vector a) where
  {-# INLINE mkEField #-}
  mkEField = param $ nonNullable $ foldableArray @Vector (nonNullable (mkEValue @a))
